...
export async function PUT(
  ...
  console.log('[API] PUT /api/admin/resources/[id]/ai-analysis: Initiated');

  try {
    const { id: resourceId } = await context.params;
    const aiAnalysis = await request.json();

    if (!resourceId) {
      return NextResponse.json({ error: 'Missing resource ID' }, { status: 400 });
    }
    if (!aiAnalysis) {
      return NextResponse.json({ error: 'Missing AI analysis data' }, { status: 400 });
    }
    ...
    // 4. Update Resource with AI Analysis using Service Role Client
    ...
    if (error) {
      console.error('[API] Error updating AI analysis:', error);
      return NextResponse.json({ error: 'Failed to update AI analysis' }, { status: 500 });
    }

    return NextResponse.json({ message: 'AI analysis updated successfully' });
