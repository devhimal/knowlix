...
export async function PUT(
  ...
  console.log('[API] PUT /api/admin/resources/[id]/plagiarism-result: Initiated');

  try {
    const { id: resourceId } = await context.params;
    const plagiarismResult = await req.json();

    if (!resourceId) {
      return NextResponse.json({ error: 'Missing resource ID' }, { status: 400 });
    }
    if (!plagiarismResult) {
      return NextResponse.json({ error: 'Missing plagiarism result data' }, { status: 400 });
    }
    ...
    // 4. Update Resource with Plagiarism Result using Service Role Client
    const { error } = await supabaseAdmin
      .from('resources')
      .update({ plagiarism_result: plagiarismResult })
      .eq('id', resourceId);

    if (error) {
      console.error('[API] Error updating plagiarism result:', error);
      return NextResponse.json({ error: 'Failed to update plagiarism result' }, { status: 500 });
    }

    return NextResponse.json({ message: 'Plagiarism result updated successfully' });
