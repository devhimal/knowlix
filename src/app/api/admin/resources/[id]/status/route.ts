...
export async function PUT(
  ...
  console.log('[API] PUT /api/admin/resources/[id]/status: Initiated');
  ...
  try {
    ...
    if (!resourceId || !status) {
      return NextResponse.json({ error: 'Missing resource ID or status' }, { status: 400 });
    }
    ...
    if (error) {
      console.error('[API] Error updating resource status:', error);
      return NextResponse.json({ error: 'Failed to update resource status' }, { status: 500 });
    }

    // You might want to fetch resource title and uploader_id here for notification
    const { data: resourceData, error: fetchResourceError } = await supabaseAdmin
      .from('resources')
      .select('title, uploader_id')
      .eq('id', resourceId)
      .single();

    if (fetchResourceError || !resourceData) {
      console.error('[API] Error fetching resource for notification:', fetchResourceError);
      // Proceed without notification if fetch fails, don't block the status update
    } else if (status === 'approved') {
      // Send approval notification (assuming addNotification is accessible or re-implemented here)
      // For now, just log, actual notification logic might need a separate API or a different approach
      console.log(`[API] Resource "${resourceData.title}" approved. Uploader: ${resourceData.uploader_id}`);
    }
